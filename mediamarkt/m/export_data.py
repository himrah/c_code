...
query = 'select pdp.category,pdp.brand,pdp.product_code,substr(pdp.product_url,24,2),pdp.category as cat,pdp.product_url,pdp.product_name,pref.markdown_price as list_price,pref.list_price as actual_price,pdp.processor,pdp.operating_system,pdp.screen_size,pdp.ram,pdp.storage,pdp.graphics,pdp.camera,pdp.bluetooth,pdp.warrenty,pdp.battery,pdp.resolution,pdp.color,pdp.block_data,pref.extraction_date as date from mediamarkt_pref as pref,mediamarkt_pdp_crawl as pdp where pref.product_code = pdp.product_code and pref.extraction_date like "%{}%"'.format(date)


cursor.execute(query)
rows = cursor.fetchall()
df = pd.DataFrame(rows)
df.insert(0,column="Retailer",value="Mediamarkt")
...
df.columns = ["Retailer","Product","Brand","Part_Number","Country","Category","Chassis","Main_page","Navigation_Second","lenovo_chassis","navigate_add_to_cart","model_name","Regular_Price","Sales_Price","dollar_off","Processor","Operating_System","Display","Memory","Hard_Drive","Graphics","camera","bluetooth","warranty","battery","Display_Resolution","colour","Details","Extraction_Date"]
df["Details"] = ""

df.to_excel("EMEA/{}_{}.xlsx".format("mediamarkt",date),index=False)